function K = apx_sk(hyp,cov,x,Vhat,opt)
...
if nargin<5, opt = []; end                           % make sure variable exists
...
function [ldB2,solveKiW,dW,dldB2,L,triB] = ldB2_exact(W,K,dK,Vhat)
  ...
  if isWneg                  % switch between Cholesky and LU decomposition mode
    ...
    sW = sqrt(W); L = chol(diag(Vhat)+sW*sW'.*K);             % Cholesky factor of B
